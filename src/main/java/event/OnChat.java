...
public class OnChat extends ListenerAdapter {
    ...
    public void onMessageReceived(MessageReceivedEvent event) {
        ...
        if (event.isFromType(ChannelType.PRIVATE)) {
            ...
            if (Settings.allowChannels.contains(event.getTextChannel().getName())) {
            ...
            boolean isHardClear = User.isHardClear(member);
            System.out.printf("[%s][%s] %s: %s , IsHardClear=%s\n", event.getGuild().getName(),
                    event.getTextChannel().getName(), event.getMember().getEffectiveName(),
                    event.getMessage().getContentRaw(), isHardClear);
             */
        }
    }
    ...
    private void test(MessageReceivedEvent event) {
        event.getChannel().sendMessage(getEmbed(event)).queue();
